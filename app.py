...
# --- CREDENCIAIS ---
# Mantenha o token atualizado. Se der erro, pegue um novo no painel da 'Apaixonada'.
...
# PASSO 1: O Cookie que você pegou no F12
...
if st.button("Consultar"):
    if mlb_input:
        ...
        try:
            # 1. PASSO: Buscar no Shopping de Preços
            ...
            if "seller_id" in res_sdp.text:
                # 2. PASSO: Pegar o campo "seller_id" do JSON/HTML
                match = re.search(r'"seller_id":\s*(\d+)', res_sdp.text)
                
                if match:
                    seller_id = match.group(1)
                    
                    # 3. PASSO: Buscar na API do Mercado Livre
                    url_ml = f"https://api.mercadolibre.com/users/{seller_id}"
                    # Tentamos primeiro sem token (público), se falhar usamos o token
